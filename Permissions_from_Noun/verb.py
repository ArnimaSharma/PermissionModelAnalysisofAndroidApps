...
dictionary_verb={}
my_cursor.execute("select stemmed_verb, permissions from testdb.semantic_analysis;")
result_verb=my_cursor.fetchall()
dictionary_verb=dict(result_verb)
...
for number_of_applications in range(len(lines)):
	...
	for token in doc:
		if token.text not in stopword_list:
			...
			if stemmed_token in dictionary_verb.keys() and (token.pos_=="VERB" or dictionary_verb[stemmed_token] in all_permissions):
				permissions_app=dictionary_verb[stemmed_token].split(",")
